...
create_schema() ->
    ...
    {atomic, ok} = mnesia:create_table(resource, [{disc_copies, [node()]}, {attributes, record_info(fields, resource)}]),    
    ...
    {atomic, ok} = mnesia:create_table(encounter, [{disc_copies, [node()]}, {attributes, record_info(fields, encounter)}]),  
